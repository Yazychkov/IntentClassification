...
class TranslatorIntent(Skill):
    ...
    def load_data(self) -> None:
        with open(os.path.join(get_data_path(), "languages.json"), 'r') as file:
            lang = json.load(file)
            self.lang_reversed = {value: key for key, value in lang.items()} #ключ и значение меняются местами
    ...
    def get_answer(self, phrase, context=None) -> str:
        ...
        if not context or not context["res_parser"].get('lang'):
            return "Проверьте правильность написания языка, на который вы хотите перевести"
        # destination_language = str()
        # phrase_list = phrase.lower().split()
        # phrase_for_leng = phrase_list[-1:] #список, элемент которого - язык, на который переводим
        # del phrase_list[-2:] #удаляем все, кроме фразы, которую надо перевести
        # phrase_str = ' '.join(phrase_list)
        # inter_result = self.tr.translate(phrase_for_leng[0], dest='en') #перевод на англ. название языка
        ...
        # else:
        #     return 'Проверьте правильность написания языка, на который вы хотите перевести'
        phrase_str = context["res_parser"]['lang']['translate']
        destination_language = context["res_parser"]['lang']['language']
        result = self.tr.translate(phrase_str, dest=destination_language) #перевод на найденный язык
        return result.text
