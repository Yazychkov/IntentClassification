        ...
        if not city:
            ...
            if  dictionary.city_dict[letter_city_name].get(city_name):
                ...
                try:
                    ...
                    params={'q': city_name, 'units': 'metric', 'lang': 'ru', 'APPID': settings.appid})
                    data = res.json()
                    answer = "Город: " + str(city)+\
                        "; На улице " + str(data['weather'][0]['description'])+\
                        "; Температура "+ str(data['main']['temp']) +\
                        "; Ощущается как "+ str(data['main']['feels_like'])
                    return answer
